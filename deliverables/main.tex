...
We cluster on recipes, but the release shares one seed label across every recipe inside a size band, and a run effect following the band would break that choice and take simulated margin coverage over 12,000 pooled replicates to 0.821 at a quarter of latent variance and 0.580 at a half, although the size-band-clustered margin interval \ci{1.030}{1.426} of Table~\ref{tab:primary}, covering 0.935 and 0.898 there, still excludes one. A permutation test that aligns default and auxiliary runs across recipes puts that effect below an anti-conservative bound of 0.044 of margin run covariance, where simulated coverage is 0.941 at a share of 0.05, and its power is 0.490 at the bound.

On the 4,755-item PolyPythias transfer runs at three sizes (Appendix~\ref{app:transport}), we fitted each configuration on three of its nine seeds and predicted the ratio on the other six. Independence predicts it with mean absolute log error 0.339, against 0.536 for the three-run estimate and 0.512 for SNAP with item noise added to both sides. Against a target pooled across configurations SNAP beats independence, 0.275 against 0.319 on margins and 0.367 against 0.523 on accuracy, and only this target resembles the recipe folds of Section~\ref{sec:prediction}.
