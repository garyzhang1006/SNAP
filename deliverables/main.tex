...
Unbiased estimates of $T_c$ and $U_c$ don't make their ratio or square root unbiased, and either pooled sum can be nonpositive in a finite sample, which our calibration simulations check. Subtracting item-sampling variance from the full-data diagonal uses every item but needs a model of item-by-run noise, while the split needs only independence between halves, and on the released runs the two agree, 1.246 \ci{1.144}{1.340} against 1.244 on margins with interval widths within 5 percent (Appendix~\ref{app:implementation}).

\begin{algorithm}[t]
\caption{SNAP measurement recipe for a fixed battery of $K$ benchmarks scored on $R$ replicate runs per configuration. The primary analysis used a random frozen split, checked against a passage-aware one, and the pre-specified test kept passage-linked items in one half.}
\label{alg:snap}
\begin{algorithmic}[1]
...
The wild interval's coverage across the twelve populations of Table~\ref{tab:coverage}, at 2,000 replicates each, runs from 0.928 to 0.954, while the configuration percentile bootstrap drops to 0.872 under recipe-shared effects. Cross-half error correlation of 0.1 and the accuracy-like null population bias the estimate down by 0.011 to 0.016, so in these populations finite-sample bias works against finding an excess.
...
\subsection{A pre-specified test on unseen tasks}
\label{sec:heldout}
...
As an exploratory result, the held-out accuracy interval excludes one while the margin interval doesn't, so which scale carries more seed signal depends on the battery. Accuracy inflation is 1.220 \ci{1.066}{1.359}, above the original ten's 1.097 at the same scope. Removing CoQA lowers held-out margin inflation to 1.090, while removing any other task leaves it between 1.120 and 1.213, and only the removal of DROP, at 1.213 \ci{1.035}{1.377}, excludes one, so the held-out estimate depends on battery composition as the original does (Appendix~\ref{app:heldout}).
...
Our cross-half estimator adapts the split-half construction of \citet{spearman1910,brown1910}, the attenuation correction of \citet{spearman1904}, and the variance components of \citet{searle1992} and generalizability theory \citep{brennan2001}, and what we add is the estimand, run covariance across benchmarks at a fixed configuration, and its measurement on released pretraining runs, which \citet{miller2024} and \citet{heineman2025} don't estimate. \citet{cheverud1988} motivates our phenotypic plug-in comparison (Table~\ref{tab:prediction}), and we borrow the covariance analogy of \citet{martin1977} without its variance components.
...
We read the accuracy result as a bound rather than as evidence of independence. The full-battery accuracy interval includes one, and at accuracy-like item noise this design detects a true ratio of 1.10 in only 0.135 of replicates, and the primary analysis gives an upper limit of 1.157 at the scored checkpoint. Exploratory cuts whose intervals exclude one, without BoolQ at 1.558, without the 26 truncated configurations at 1.120, at the adjacent checkpoint and on the held-out tasks, don't settle it.

Apart from the pre-specified test, which failed at its pre-specified scope, and the two PolyPythias rules fixed before scoring, our study remains exploratory. We didn't score the 375 DataDecide runs on the second bank, which leaves the 1.244 untested on fresh items and the shared-item check unrun in the family that supplies our headline, although the fixed-bank value of 1.237 (Section~\ref{sec:estimator}) needs no such check for the released bank. Our populations stop at 1B and at likelihood-scored multiple choice, since no larger public multi-seed population shares replicate item scores (Appendix~\ref{app:design}) and generative tasks lack our margin. On the released item noise a uniform item effect shared across benchmarks can't produce the margin value (Section~\ref{sec:estimator}), while on accuracy a share of 0.053 of item-noise variance would reproduce 1.078 and the accuracy cross-format interval \ci{0.848}{1.063} doesn't exclude the 1.047 it implies, and a component that follows scoring format remains unmeasured in DataDecide (Appendix~\ref{app:calibration}).\outcome{\Rtwocase}{1}{R2 not supported}{ The cross-bank check in PolyPythias constrains that item effect in one family, and it can't reach an item effect that the two banks share.}\outcome{\Rtwocase}{2}{R2 supported}{ The cross-bank check in PolyPythias finds such a component in one family, so the within-bank value can overstate run covariance there.}\outcome{\Rtwocase}{3}{R2 undecided}{ The cross-bank check in PolyPythias leaves that item effect undecided in one family.}

We cluster on recipes, but the release shares one seed label across every recipe inside a size band, and a run effect following the band would break that choice and take simulated margin coverage over 10,000 replicates to 0.823 at a quarter of latent variance and 0.581 at a half, although the size-band-clustered margin interval \ci{1.030}{1.426} of Table~\ref{tab:primary} still excludes one. A permutation test that aligns runs by batch slot bounds that effect at 0.044 of the seed covariance on margins, where simulated coverage is 0.941 at a share of 0.05, and that test's power is 0.490 at the bound.

For a fixed released bank the replicate standard deviation of the battery average gives the aggregate uncertainty directly, and we recommend reporting it with a benchmark-removal check, while the split adds the item-general value for a redrawn or extended bank, the per-benchmark run variances, and $K_{\mathrm{eff}}$ for planning run counts. Against one configuration's six-run margin ratio on the 4,755-item three-size PolyPythias transfer runs of Appendix~\ref{app:transport}, whose middle 90\% runs from 0.70 to 1.85, independence predicts with mean absolute log error 0.339 against 0.536 for the plug-in and 0.512 for our decomposition, while against a target pooled across configurations at three sizes the decomposition beats independence, 0.275 against 0.319 on margins and 0.367 against 0.523 on accuracy, and only this pooled target resembles the recipe folds of Section~\ref{sec:prediction}. The ratio itself belongs to this battery. Without replicates, the format fit of Appendix~\ref{app:formatfit} gives a fallback scaling untested outside this battery. Scaling the independence standard error by a ratio from the same margin population returns the simulated false-positive rate from 0.113 to 0.051. In a population with the released item counts and equicorrelated seed covariance at the observed mean correlation, three runs each let the margin interval exclude one in 0.997 of replicates, and accuracy needs eight to reach 0.803.
