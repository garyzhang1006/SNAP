...
3. **Unlock the gated checkpoints.** Log in to Hugging Face, accept access terms on the gated DataDecide repositories, and store the token as a Kaggle secret named `HF_TOKEN`, never in any file. Resolve the five `dolma1.6++` identifiers by reading the organisation listing, not by guessing names.
