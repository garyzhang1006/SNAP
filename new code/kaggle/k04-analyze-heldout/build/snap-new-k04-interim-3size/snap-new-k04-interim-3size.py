...
# Interim build (PROTOCOL_heldout.md, 2026-09-19 interim entry): exploratory,
# not the pre-registered test. 1B is partial, so incomplete configurations are
# dropped by build_population and the missing runs are logged, not asserted.
sizes_scored = ['530M', '750M', '1B']
...
log["notes"].append("interim build at 530M, 750M and the scored 1B configurations; not the pre-registered test")
log["notes"].append(f"{len(missing)} runs at the scored sizes have no scores yet and are absent: {missing}")
...
# Same recipe-by-size cells as the held-out population, so the comparison at
# 1B covers the scored configurations rather than all 25 recipes.
from collections import Counter
n_cell = Counter((runs_cfg[k]["release_recipe"].lower(), runs_cfg[k]["size"]) for k in files)
cells = {c for c, n in n_cell.items() if n == 3}
same_dir = Path("/kaggle/tmp") / "runs_shipped_same_cells"
shutil.rmtree(same_dir, ignore_errors=True)
same_dir.mkdir(parents=True)
for p in sorted(old_dir.glob("*.npz")):
    _, m_ = load_run(p)
    if (m_["recipe"].lower(), m_["size"]) in cells:
        shutil.copy(p, same_dir / p.name)
res_old_same_sizes = headline(build_population(same_dir, TRAITS, n_runs=3)[0], "original-traits-same-cells")
log["comparison_cells"] = sorted(list(c) for c in cells)
