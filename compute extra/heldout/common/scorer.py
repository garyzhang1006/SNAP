"""Log-likelihood scoring of multiple-choice requests with a DataDecide checkpoint.

The scoring rule copies the OLMES Hugging Face path (lm_eval 0.4.3
HFLM._encode_pair and loglikelihood) so the new numbers mean what the released
DataDecide numbers mean:
...
All choices of one item share the context tokens, so the context runs once per
item and its key/value cache feeds every choice. Contexts are left-padded in
packed batches. OLMo turns the padding mask into a finite additive bias
(olmo/model.py, ensure_finite_) and uses rotary positions, which depend only on
token distance, so padding changes real-token scores only by floating-point
noise. That claim is tested, not trusted: before a run is scored, a spread of
check items is scored both ways, and the run uses the fastest path that agrees
with the one-sequence reference path within kv_check_tolerance_nats. Packed
batches are tried first, then one item at a time without padding, then the
reference path for everything.
"""
...
def load_model(local_dir, dtype, device):
    """Returns (inner OLMo module, tokenizer). The inner module exposes
    last_logits_only, which keeps the context pass from materialising a
    batch x length x vocabulary logit tensor."""
    from hf_olmo import OLMoForCausalLM
    from transformers import AutoTokenizer

    hf = OLMoForCausalLM.from_pretrained(local_dir, torch_dtype=getattr(torch, dtype), attn_implementation="sdpa")
    hf.to(device)
    hf.train(False)
    if hf.model.config.flash_attention:
        raise RuntimeError("flash_attention is still on after attn_implementation='sdpa'; T4 cards can't run it")
...
def score_reference(model, ctx, cont, max_length, device):
    ...
    logits = model(input_ids=inp).logits[0, -len(cont):].float()
...
def score_packed(model, items, pad_id, device):
    ...
    Returns one list of sum_logits per item."""
    B = len(items)
    ...
    out = model(input_ids=ctx_ids, attention_mask=ctx_mask, use_cache=True, last_logits_only=True)
    first_logp = F.log_softmax(out.logits[:, -1].float(), dim=-1)
    ...
    if C > 1:
        ...
        past = [(k.index_select(0, idx), v.index_select(0, idx)) for k, v in out.attn_key_values]
        mask = torch.cat([ctx_mask.index_select(0, idx), cont_mask], dim=1)
        rest = model(input_ids=cont_ids, attention_mask=mask, past_key_values=past).logits
    for n, c in enumerate(rows):
...
def score_requests(model, tokenizer, requests, cfg, device, log=print):
    """Score every choice of every request, in file order.
    Returns (float64 array of sum_logits, stats dict)."""
    pad_id = tokenizer.pad_token_id if tokenizer.pad_token_id is not None else 1
    encoded = []
    for r in requests:
        pairs = [encode_pair(tokenizer, r["context"], c) for c in r["continuations"]]
        ctx = pairs[0][0]
        if any(p[0] != ctx for p in pairs):
            raise ValueError(f"{r['task']} doc {r['doc_id']}: choices tokenise their shared context differently")
        if any(len(p[1]) == 0 for p in pairs):
            raise ValueError(f"{r['task']} doc {r['doc_id']}: a continuation encodes to zero tokens")
        encoded.append((ctx, [p[1] for p in pairs]))
    order = sorted(range(len(requests)), key=lambda i: -len(encoded[i][0]))
    ...
    for candidate in MODES[:-1]:
        ...
        worst = max(abs(got[j][k] - ref[j][k]) for j in check for k in range(len(ref[j])))
    ...
    flat = np.asarray([v for j in range(len(requests)) for v in out[j]], dtype=np.float64)
    ...
    return flat, {"mode": mode, "check_items": n_check, "check_max_abs_diff_nats": diffs,
                  "items": len(requests), "choices": int(flat.size), "tokens_total": int(tokens),
