...
def wild_t(T, U, cluster, weights="rademacher", log=False, n_boot=DRAWS, alpha=0.05, seed=SEED):
    ...
    return {"lo": sqrt_or_nan(lo), "hi": sqrt_or_nan(hi), "finite_draws": int(tb.size), "distinct_t_values": distinct,
...
def twoway_cluster_t(T, U, recipe, band, alpha=0.05):
    ...
    T, U = np.asarray(T, float), np.asarray(U, float)
    th = _theta(T, U)
    v = cluster_se(T, U, recipe) ** 2 + cluster_se(T, U, band) ** 2 - cluster_se(T, U, np.arange(T.size)) ** 2
    G = min(np.unique(recipe).size, np.unique(band).size)
    if v <= 0:
        return {"lo": None, "hi": None, "se": None, "variance": float(v), "method": f"two-way cluster t({G - 1}), variance not positive"}
    se = float(np.sqrt(v))
    q = stats.t.ppf(1 - alpha / 2, G - 1)
    return {"lo": sqrt_or_nan(th - q * se), "hi": sqrt_or_nan(th + q * se), "se": se, "df": G - 1,
...
for phen in ("margin", "accuracy"):
    for ds, job in (("full", "original"), ("noboolq", "noboolq_original")):
        ...
        cell = {"lambda": sqrt_or_nan(_theta(T, U)),
                "recipe_wild": as_row(wild_bootstrap_t(T, U, recipe, n_boot=DRAWS, seed=SEED)),
...
for phen in ("margin", "accuracy"):
    ...
    est = {"n": 33, "recipes": int(n_recipes), "by_size": {s: int((band == s).sum()) for s in BANDS},
           "lambda": sqrt_or_nan(_theta(T, U)), "recipe_wild": as_row(wild_bootstrap_t(T, U, recipe, n_boot=DRAWS, seed=SEED)),
           "recipe_wild_webb": wild_t(T, U, recipe, weights="webb")}
...
for name, pop in POPULATIONS.items():
    ...
    excl, widths, lows, undefined, point = 0, [], [], 0, []
    ...
    defined = POWER_REPS - undefined
    block_b["power"][name] = {"true_lambda": true_lambda, "rho": pop["rho"], "noise_sd": pop["noise_sd"], "reps": POWER_REPS, "draws": POWER_DRAWS,
                              "undefined_intervals": undefined, "exclusion_rate_lower_above_one": excl / defined if defined else None,
                              ...
                              "note": "design is the 33 matched configurations with their 24 recipe clusters; no recipe-shared or band-shared component"}
...
for phen in ("margin", "accuracy"):
    ...
    strat = json.load(open(src))
    cell = {"stratified_original_lambda": block_a[f"{phen}/full"]["lambda"], "nostrata_original": {"lambda": base["lambda"], "offdiag": offdiag(base, meta["weights"]),
                                                                        "wild": base["uncertainty"].get("interval")},
    ...
    for key in ("lambda", "offdiag"):
        ...
        cell[f"{key}_comparison"] = {"subject_mean": float(s.mean()), "subject_sd": float(s.std(ddof=1)), "item_mean": float(i.mean()),
                                     "item_sd": float(i.std(ddof=1)), "difference_in_pooled_sd": float((s.mean() - i.mean()) / pooled) if np.isfinite(pooled) and pooled > 0 else None,
    ...
    cell["note"] = ("MMLU scored as a plain item mean in every row of this block, so the nostrata baseline differs from the "
                    ...
                    "drawn just before it, so the two arms differ only in whether whole subjects move together")
    block_c[phen] = cell
