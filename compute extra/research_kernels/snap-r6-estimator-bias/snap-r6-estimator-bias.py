"""R6 how far the point estimate itself sits from the truth, across the range the paper works in.

Design fixed before running (2026-09-16, written 16:00 EDT). The estimator is a square root of a ratio of
two sums, and a ratio of sums is biased in finite samples even when both sums are unbiased. The paper
reports 1.2439 and 1.0784 as point estimates and spends its care on the intervals around them, which
leaves a reader no way to know whether the point itself runs high or low at this design. The square root
adds a second source, since it is concave and pulls the mean of the root below the root of the mean.
This run measures both at once. It sweeps the true ratio over eight values from 1.00 to 1.40 at the
margin battery's item noise and again at the accuracy battery's, recording the mean and standard
deviation of the estimate against the truth alongside the coverage, at 4,000 replicates per cell. A bias
that stays inside a few thousandths across the range says the point estimates can be read as written.
A bias that grows toward the lower end would matter most for the accuracy conclusion, which sits there.
Seed 20260973 keeps the draws distinct.
"""
...
# rho solves Lambda^2 = 1 + (k - 1) rho at ten benchmarks with flat weights.
# rho solves Lambda^2 = 1 + 9 rho at ten benchmarks with flat weights.
CELLS = [
 {"name": "margin_lambda_1.00", "rho": 0.0, "noise_sd": 0.73},
 {"name": "accuracy_lambda_1.00", "rho": 0.0, "noise_sd": 1.45},
 {"name": "margin_lambda_1.05", "rho": (1.05 ** 2 - 1) / 9, "noise_sd": 0.73},
 {"name": "accuracy_lambda_1.05", "rho": (1.05 ** 2 - 1) / 9, "noise_sd": 1.45},
 {"name": "margin_lambda_1.10", "rho": (1.10 ** 2 - 1) / 9, "noise_sd": 0.73},
 {"name": "accuracy_lambda_1.10", "rho": (1.10 ** 2 - 1) / 9, "noise_sd": 1.45},
 {"name": "margin_lambda_1.15", "rho": (1.15 ** 2 - 1) / 9, "noise_sd": 0.73},
 {"name": "accuracy_lambda_1.15", "rho": (1.15 ** 2 - 1) / 9, "noise_sd": 1.45},
 {"name": "margin_lambda_1.25", "rho": (1.25 ** 2 - 1) / 9, "noise_sd": 0.73},
 {"name": "accuracy_lambda_1.25", "rho": (1.25 ** 2 - 1) / 9, "noise_sd": 1.45},
 {"name": "margin_lambda_1.40", "rho": (1.40 ** 2 - 1) / 9, "noise_sd": 0.73},
 {"name": "accuracy_lambda_1.40", "rho": (1.40 ** 2 - 1) / 9, "noise_sd": 1.45},
]
...
for ci, cell in enumerate(CELLS):
    ...
    for rep in range(REPS):
        ...
        _th = float(np.sum(T)) / float(np.sum(U))
        hats.append(float(np.sqrt(_th)) if _th > 0 else float("nan"))
        ...
        for m, (lo, hi) in ivs.items():
            if np.isfinite(lo) and np.isfinite(hi):
                ...
                acc[m]["bad"] += 1
    _h = np.asarray([x for x in hats if np.isfinite(x)], float)
    out = {"name": cell["name"], "truth": truth,
           "lambda_hat_mean": float(_h.mean()), "lambda_hat_sd": float(_h.std(ddof=1)),
           "lambda_hat_bias": float(_h.mean() - truth),
           "undefined_estimates": 1.0 - _h.size / REPS,
...
(W / "r6_estimator_bias.json").write_text(json.dumps(report, indent=1))
