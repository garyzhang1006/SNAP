...
def main():
    ...
    ap.add_argument("--exclude", nargs="*", default=[], help="trait names to drop from the battery")
    args = ap.parse_args()
    ...
    unknown = [t for t in args.exclude if t not in TRAITS]
    assert not unknown, f"unknown traits to exclude: {unknown}"
    benchmarks = []
    per_trait_idx = []
    for j, name in enumerate(TRAITS):
        ...
        benchmarks.append(b)
    kept = len(benchmarks)
    layout = {"configs": configs, "benchmarks": benchmarks, "weights": [1.0 / kept] * kept,
              "excluded_traits": list(args.exclude), "grouped_traits": sorted(groups),
    ...
    summary = {"configs": len(configs), "runs": len(runs), "items": int(ref.n_items),
               ...
               "items_per_trait": {b["name"]: int(per_trait_idx[j].size) for j, b in enumerate(benchmarks)},
               "written": written}
