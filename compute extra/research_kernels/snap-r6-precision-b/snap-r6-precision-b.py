"""R6 precision replication part b, the remaining eight coverage cells at 10,000 replicates.

Design fixed before running (2026-09-15, written 19:40 EDT). Part a covered the two cells calibrated
to the observed estimates and the two with the lowest wild coverage. This part covers the other eight
cells of the coverage table so every cell has a Monte Carlo standard error near 0.0022 rather than
0.005. The generator, the seed 20260917 and the four interval procedures are those of part a, with
the cell index offset by ten so no dataset is shared with part a.
"""
...
CELLS = [
 {"name": "null_margin_noise", "rho": 0.0, "noise_sd": 0.73},
 {"name": "null_accuracy_noise", "rho": 0.0, "noise_sd": 1.45},
 {"name": "margin_like_boolq_trace", "rho": 0.061, "noise_sd": 0.73, "trait_scales": [4.373213921133976] + [1.0] * 9},
 {"name": "accuracy_like_boolq_trace", "rho": 0.018, "noise_sd": 1.45, "trait_scales": [6.873863542433759] + [1.0] * 9},
 {"name": "margin_like_recipe_shared", "rho": 0.061, "noise_sd": 0.73, "recipe_shared": 0.5},
 {"name": "margin_like_heavy_tails", "rho": 0.061, "noise_sd": 0.73, "student_df": 4},
 {"name": "accuracy_like_cross_half_error_corr_0.1", "rho": 0.018, "noise_sd": 1.45, "cross_half_error_correlation": 0.1},
 {"name": "rho_0.2_margin_noise", "rho": 0.2, "noise_sd": 0.73},
]
...
SEED, REPS = 20260917, 10000
report = {"design": __doc__, "seed": SEED, "reps": REPS, "cells": {}}
for ci, cell in enumerate(CELLS):
    t1 = time.time(); acc = {m: {"cov": 0, "lo": 0, "up": 0, "bad": 0, "w": []} for m in ("wild", "cluster_t", "centred", "restricted")}
    for rep in range(REPS):
        rng = np.random.default_rng([SEED, ci + 10, rep])
        T, U, recipe, truth = r6_simulate(cell, rng)
        ivs = {}
        try:
            wb = wild_bootstrap_t(T, U, recipe, n_boot=4999, seed=rep); ivs["wild"] = (wb.lo, wb.hi)
        except Exception:  # noqa: BLE001
            ivs["wild"] = (float("nan"), float("nan"))
        ct = cluster_t_interval(T, U, recipe); ivs["cluster_t"] = (ct.lo, ct.hi)
        for m, (lo, hi) in ivs.items():
            if np.isfinite(lo) and np.isfinite(hi):
                acc[m]["cov"] += lo <= truth <= hi; acc[m]["lo"] += lo > truth; acc[m]["up"] += hi < truth; acc[m]["w"].append(hi - lo)
            else:
                acc[m]["bad"] += 1
        for form in ("centred", "restricted"):
            res = invert(T, U, recipe, form=form)
            acc[form]["cov"] += covers(res, truth)
            if res["kind"] == "bounded":
                lo, hi = res["lambda_sets"][0]; acc[form]["lo"] += lo > truth; acc[form]["up"] += hi < truth; acc[form]["w"].append(hi - lo)
            else:
                acc[form]["bad"] += 1
    out = {"name": cell["name"], "truth": truth}
    for m, a in acc.items():
        c = a["cov"] / REPS
        out[m] = {"coverage": c, "mc_se": float(np.sqrt(c * (1 - c) / REPS)), "lower_miss": a["lo"] / REPS, "upper_miss": a["up"] / REPS,
                  "undefined_or_unbounded": a["bad"] / REPS, "median_width": float(np.median(a["w"])) if a["w"] else None}
    report["cells"][cell["name"]] = out
    print(f"[{cell['name']}] {time.time() - t1:.0f}s {json.dumps(out)}", flush=True)
report["wall_seconds"] = time.time() - t0
(W / "r6_precision_b.json").write_text(json.dumps(report, indent=1))
