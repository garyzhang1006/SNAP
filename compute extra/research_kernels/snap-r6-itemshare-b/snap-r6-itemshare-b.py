...
This is the b run. The a run used 200 permutations, which puts a Monte Carlo standard error of 0.019
on a tail share near 0.075, and that share now carries a claim in the main text. This run repeats the
same design with 2,000 permutations and a new permutation seed, and it drops the inner split loop that
the a run already measured. The half splits themselves are redrawn again as an independent check.
"""
...
(W / "r6_itemshare_b.json").write_text(json.dumps(report, indent=1))
