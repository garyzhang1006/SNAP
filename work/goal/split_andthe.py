"""Scan 117 helper: propose splitting ", and the <clause>" in an appendix file into two sentences.
...
SKIP = next((a.split("=")[1].split(",") for a in sys.argv if a.startswith("--skip=")), [])
...
out, i, n = [], 0, 0
for m in re.finditer(r", and (?=the )", s):
    pre = s[:m.start()]
