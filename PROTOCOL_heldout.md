...
CoQA is now a primary task, and its fallback grouping gives one group per item, which would let questions about the same story land in both halves of the split. Group labels only enter at analysis time, so before k04 reads any production score we will regroup coqa_mc by story with a deterministic key over the story text, record the new request hash here, and leave the scored prompts untouched.
