...
## Kernel r32 (written 2026-09-21, unrun)

`compute extra/research_kernels/snap-r32-cluster-power/` covers the fourth panel's three items in one private CPU kernel on the shipped moments plus the reduced runs: (A) size-band clustering with Rademacher and Webb six-point wild bootstrap-t on theta and log theta, a cluster-robust t(4), and a two-way recipe-by-band cluster-robust t, on both scales with and without BoolQ, after reproducing the recipe-clustered intervals to 1e-6; (B) the 33 shared-final-step configurations re-estimated with their 24 recipe clusters (reproducing 1.082 with 0.121 to 1.530) and a 2,000-replicate power simulation at that exact design under the margin-like and accuracy-like populations of the r6 calibration cells, reporting the rate at which the wild interval's lower endpoint clears one; (C) MMLU scored as a plain item mean with 200 whole-subject splits against 200 item re-splits of matched side sizes, both scales, reporting the two distributions of the inflation factor and the off-diagonal contribution with the variance ratio and its F tail as the primary statistic. After its first review the kernel reproduces the shipped intervals at seed 0 (the matched-step lower endpoint 0.121 moves by tenths across seeds because theta's lower bound sits near zero, and the kernel records four other seeds beside it), reports the exclusion rate over all replicates with the undefined share beside it, adds two null populations at rho 0 to calibrate the size at the 33-configuration design, falls back to the larger one-way variance when the two-way variance is negative (which it is for margin on the full battery), and carries the seednoise guards, point and standard error in every interval row. Launch after approval with the same push command and the directory name changed to snap-r32-cluster-power. Runtime is dominated by 160 unbootstrapped C01 jobs and 4,000 vectorised wild bootstraps, under forty minutes with the 800 subject and item jobs.
