"""Collective style profile of corpus3 (twelve accepted 2018-2021 papers, main text only)
...
def snap_text(part='all'):
    ...
    return body
rows=[]
for p in sorted(Path('corpus3').glob('*.txt')):
...
print(f"{'CORPUS3 MEDIAN (n=%d)'%len(rows):32s}"+''.join(fmt(med[k]) for k in keys))
print(f"{'CORPUS3 MIN':32s}"+''.join(fmt(lo[k]) for k in keys))
print(f"{'CORPUS3 MAX':32s}"+''.join(fmt(hi[k]) for k in keys))
print(f"{'corpus2 median (n=%d)'%len(c2):32s}"+''.join(fmt(med2[k]) for k in keys))
print(f"{snap['name']:32s}"+''.join(fmt(snap[k]) for k in keys))
print(f"{snapi['name']:32s}"+''.join(fmt(snapi[k]) for k in keys))
print('\nOUTSIDE CORPUS3 RANGE (SNAP main text):')
for k in keys:
    if k in ('sents','words'): continue
    v=snap[k]
    if v<lo[k] or v>hi[k]: print(f"  {k:10s} SNAP {v}  corpus3 median {med[k]}  range [{lo[k]}, {hi[k]}]")
