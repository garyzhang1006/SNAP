...
# Exploratory per-size build (PROTOCOL_heldout.md lists any per-size breakdown
# as reported but not part of the test): the held-out estimator on the 75 runs
# at 750M alone, to sit beside the 530M-only interim and the 530M+750M result.
sizes_scored = [s for s in sizes_scored if s == "750M"]
assert sizes_scored == ["750M"], f"expected 750M fully scored, got {sizes_scored}"
log["notes"].append("exploratory per-size run at 750M only; not the registered test")
